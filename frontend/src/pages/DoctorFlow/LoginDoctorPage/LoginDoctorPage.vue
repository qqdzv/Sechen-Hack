...
import LoginStep1 from './LoginSteps/LoginDocStep1.vue';
import LoginStep2 from './LoginSteps/LoginDocStep2.vue';
...
const handleLogin = async () => {
    try {
        const response = await api.postData<PostData, PostResponse>('/doctor/login', { email: email.value, password: password.value });
        postResponse.value = response;
        router.push(`/doctor/chats`); // Устанавливаем значение postResponse
    } catch (error: any) {
